  ...
  try {
    if (currentChatId) {
      ...
      try {
        ...
        if (
          messagesToSave.length === 2 &&
          messagesToSave[1]?.role === "assistant"
        ) {
        ...
        console.log(
          "Chat update failed, creating new chat instead. Error:",
          updateError,
        );
  ...
  for (const file of files) {
    try {
      ...
      if (file.size > MAX_FILE_SIZE) {
        console.warn(
          `File ${file.name} is too large (${(file.size / 1024 / 1024).toFixed(2)}MB). Max size is 5MB.`,
        );
        continue;
      }
...
function determineFileType(
  ...
  if (
    ...
    configFiles.some((name) => fileName.includes(name))
  ) {
    return "config";
  }
...
async function processPdfFile(file: File): Promise<Attachment> {
  console.log(
    "Sending PDF to extract endpoint:",
    file.name,
    file.type,
    file.size,
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(
        "PDF extract API failed, using fallback:",
        response.status,
        response.statusText,
        errorText,
      );
