  ...
  try {
    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error("DALL-E request timeout after 45 seconds")),
          45000,
        ),
      ),
    ]);
    ...
    console.log(
      `📦 Base64 image size: ${Math.round(base64Image.length / 1024)}KB`,
    );
    ...
    console.log(
      `🏁 Total image generation time: ${Date.now() - totalStartTime}ms`,
    );
